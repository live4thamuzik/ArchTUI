...
pub fn render_loading_screen(f: &mut Frame, state: &AppState) {
    ...
    let spinner_frames = ["-", "\\", "|", "/"];
    // Use installation_progress as a simple tick counter for the spinner
    let spinner_frame = spinner_frames[(state.installation_progress as usize / 5) % spinner_frames.len()];
