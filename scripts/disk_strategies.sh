...
do_manual_partitioning_guided() {
    ...
    log_warn "2. If using UEFI, create an EFI System Partition (type EF00) for /efi"
    log_warn "3. If using UEFI, create an Extended Boot Loader Partition (type EA00) for /boot"
    log_warn "4. Set the root partition type to Linux (type 8300)"
    log_warn "5. Write changes with 'w' and quit fdisk"
    log_warn "6. The script will then ask you to format and mount partitions"
    ...
    log_warn "If using UEFI, create and mount the EFI System Partition at '/mnt/efi'."
    log_warn "If using UEFI, create and mount the Extended Boot Loader Partition at '/mnt/boot'."
    log_warn "If using LVM, LUKS, or RAID, ensure they are opened/assembled before mounting."

    read -rp "Press Enter when you have finished formatting and mounting to /mnt (and /mnt/efi, /mnt/boot): "

    # Verify essential mounts
    if ! mountpoint -q /mnt; then
        error_exit "/mnt is not mounted. Please ensure your root partition is mounted correctly."
    fi
    if [ "$BOOT_MODE" == "uefi" ] && ! mountpoint -q /mnt/efi; then
        log_warn "/mnt/efi is not mounted. This is required for UEFI installations. Please mount it manually."
    ...
    if [ "$BOOT_MODE" == "uefi" ] && ! mountpoint -q /mnt/boot; then
        log_warn "/mnt/boot is not mounted. This is required for UEFI installations. Please mount it manually."
    ...
    # Process EFI
    if [ "$BOOT_MODE" == "uefi" ]; then
        local efi_dev=$(echo "$mounted_devs_info" | awk '$2=="/mnt/efi"{print $1}')
        if [ -n "$efi_dev" ]; then
...
# --- Missing RAID Partitioning Functions ---
